...
import 'package:cakery/screens/home/Chef.dart';

class ChefLoginPage extends StatefulWidget {
  const ChefLoginPage({super.key});

  @override
  _ChefLoginPageState createState() => _ChefLoginPageState();
}
      ...
      try {
        ...
        if (userCredential.user != null) {
          ...
          if (snapshot.snapshot.exists) {
            print('Chef snapshot data: ${snapshot.snapshot.value}');
            // Chef is valid, navigate to the chef dashboard
            Navigator.pushReplacement(
              context,
              MaterialPageRoute(builder: (context) => CakeOrderApp()),
            );
          } else {
            await _auth.signOut();
            _showSnackBar('This account is not registered as a chef');
          }
        }
    ...
    return SafeArea(
      child: Scaffold(
        appBar: AppBar(
          ...
          title: Text('Chef Login', style: TextStyle(color: Colors.black)),
        ),
